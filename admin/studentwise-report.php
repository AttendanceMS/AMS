...
<!-- /Sidebar -->
 ...
 $tdate=$_SESSION['todate'];
  $claname=$_SESSION['claname'];
   $stdname= $_SESSION['sname'];
$sql = "SELECT studentName from  tblstudents where id=:stdname";
...
<h4 class="hk-pg-title"><span class="pg-title-icon"><span class="feather-icon"><i data-feather="toggle-right"></i></span></span>Attendance Report of <?php echo htmlentities($studentname)?> <?php echo htmlentities($claname)?> class student From <?php echo htmlentities($fdate);?> To <?php echo htmlentities($tdate);?></h4>
                </div>
                <!-- /Title -->
...
max(case when  studentId=:stdname and attndShift=2 and studentClass=:clss and attndDate between :fdate and :tdate then tblattendance.attndStatus end) as s2,
max(case when  studentId=:stdname and attndShift=1 and studentClass=:clss and attndDate between :fdate and :tdate then tblattendance.attndStatus end) as s1
